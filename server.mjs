...
async function importWorks(incoming, projectId=''){ const file=path.join(root,'data','works.json'), data=JSON.parse(await fs.readFile(file,'utf8')),activeProject=clean(projectId)||data.projectId||'default'; const key=w=>(w.landingUrl?.match(/10\.\d{4,9}\/[\w.()/:;-]+/i)||[])[0]?.toLowerCase()||String(w.title||'').toLowerCase().replace(/\W/g,''); const map=new Map(data.works.map(w=>[key(w),w])); incoming.filter(w=>w&&w.title).forEach(w=>{const k=key(w),old=map.get(k),incomingProject=clean(w.projectId)||activeProject; if(!old){map.set(k,{...w,projectId:incomingProject});return;} const merged={...old,...w,projectId:incomingProject,abstract:w.abstract||old.abstract,concepts:[...new Set([...(old.concepts||[]),...(w.concepts||[])]) ]}; if(old.projectId&&old.projectId!==incomingProject&&!w.markerColor){merged.markerColor='default';merged.markerProjectId=incomingProject;} map.set(k,merged);}); data.works=[...map.values()]; data.projectId=activeProject; data.mergedAt=new Date().toISOString(); await fs.copyFile(file,`${file}.bak`).catch(()=>{}); await fs.writeFile(file,JSON.stringify(data,null,2)); await new Promise((resolve,reject)=>{ const p=spawn(process.execPath,['scripts/build-graph.mjs'],{cwd:root}); let stderr='';p.stderr?.on('data',chunk=>stderr+=chunk);p.on('error',reject);p.on('exit',c=>c?reject(new Error(`重新建圖失敗（代碼 ${c}）：${clean(stderr).slice(0,300)}`)):resolve()); }); return data.works.length; }
...
const server=http.createServer(async(req,res)=>{ try{
  ...
  if(req.method==='POST'&&req.url==='/api/import'){const b=await body(req);return json(res,200,{total:await importWorks(b.works||[],b.projectId)});}
